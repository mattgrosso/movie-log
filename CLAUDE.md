...
Router: `/games` (hub) + `/games/rate-off` / `/games/higher-lower` / `/games/wordle` / `/games/connections` / `/games/six-degrees`, all `requiresLogin`. Entry point: a controller-icon button in Home's results-actions row (`goToGames()`) — see the Grouped Result Hierarchy section above for why it moved into the slot the group-order button used to occupy.
...
Tests: `src/test/games/` — pure-module tests (`gameUtils`, `bracket`, `wordleClues`, `connectionsGenerator`, `sixDegrees`, no mounting) plus one mount-based test per game component. Two recurring test-writing traps worth remembering if you add another game: (1) `new Date('YYYY-01-01')` parses as UTC midnight and shifts to the previous year/decade in this repo's negative-UTC-offset test environment — use a mid-month date in fixtures, not the 1st; (2) if a component sets its own initial state in `mounted()` rather than a computed or `created()`, a synchronous post-mount DOM assertion will see the pre-mount-hook render, one tick stale — prefer `created()` for pure data setup that doesn't touch the DOM. (3) `gameData.js`'s `eligibleGameEntries` reads `this.$store.state?.settings?.includeShorts` — optional-chained on `state` itself, not just `settings`, because several game test mocks stub `$store` without a `state` object at all; don't tighten that back up without also adding `state: {}` to every affected mock.

**Not built yet** (designed, deliberately deferred): per-game filters (e.g. restrict a Rate-Off bracket to one genre/decade/quick-link before starting), a shareable Wordle-style result card for Reel Wordle, and a persisted hall-of-fame of past Rate-Off champions.
...
- **New pure module: `src/assets/javascript/tieBreakTournament.js`** (store-free, same pattern as `games/bracket.js`) — `findTiedGroup(sortedEntries, getScore)` expands outward from the first adjacent tie to the full **contiguous run** of equal-scored entries (ties are always contiguous in a sorted list); `createRoundRobinTournament(contestantIds)` builds every unique pair (N choose 2, no shuffling — deterministic); `currentMatch`/`recordMatchResult`/`isComplete` are an immutable state machine identical in spirit to `bracket.js`'s `currentPair`/`pickWinner`; `rankContestants` sorts by win count descending (falls back to original order for a tie-of-ties, e.g. a 3-cycle where everyone goes 1-1 — not worth resolving further); `tweakDeltaForRank(rank)` generalizes the old flat `-0.1` to `-0.1 * rank` (rank 0 = untouched, matching "winner never penalized").
