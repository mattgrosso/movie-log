...
- **Higher or Lower** (`HigherLowerGame.vue`) — classic "here's a real score, is the next one higher or lower" streak game. Ties count as a correct guess either way. Best streak persists to `settings/games/higherLowerBestStreak` via the normal `setDBValue` path (cross-device, like everything else in `settings`). Advancing to the next challenger after a correct guess has a ~900ms delay (so the revealed score is actually readable) — tests use `vi.useFakeTimers()`.
- **Reel Wordle** (`wordleClues.js` + `gameUtils.pickDailyEntry` + `ReelWordleGame.vue`) — one movie from the library, chosen **deterministically per calendar day** (`hashString(dateString)` seeds a small mulberry32 PRNG, indexes into the eligible-entries array — no server round trip, same puzzle for the same day on repeat visits). 6 guesses via a type-ahead over the user's own titles; each guess is scored against the target on year/decade/director/genre-overlap/runtime/your-rating, Wordle-style (↑ guess too low, ↓ too high, ✓ match). **Progress persists in `localStorage`** (`cinemaRoll.reelWordle.<dateString>`, just the ordered list of guessed entry keys — clues are recomputed on load, not stored, so a change to the clue logic doesn't desync old saved games) — intentionally NOT Firebase, since it's disposable same-day state, not a durable record.
- **Taste Quiz** (`quizGenerator.js` + `TasteQuizGame.vue`) — auto-generated multiple-choice trivia about the user's own aggregated stats (top director/genre/actor/release-year by count, "which of these 4 did you rate highest"). `topCountQuestion()` deliberately **bails (returns null) rather than guess** when the data can't support an unambiguous answer — fewer than 4 distinct values, or a tie for first place — so a small/homogeneous library never produces a wrong "correct" answer. The quiz always tries each category type once, then pads out to the requested question count with fresh "highest rated" instances (which only need 4 movies and essentially never tie), so quiz length is reliable even for smaller libraries.
- **Connections** (`connectionsGenerator.js` + `ConnectionsGame.vue`) — NYT Connections-style: find 4 groups of 4 movies each sharing a director/genre/decade/cast member/studio. `buildCandidateCategories` finds every attribute value shared by 4+ library movies; `generateConnectionsPuzzle` shuffles those candidates and greedily claims 4 with **no movie assigned to more than one group's answer key** (a movie CAN be individually eligible for several candidates at once — that's real puzzle difficulty, not a bug — the constraint is only on which category actually *claims* it). 4 mistakes allowed. `status`'s loss check is deliberately ordered before its win check — a loss reveals every category via the same "solved" list the win path uses, which would otherwise also satisfy the win condition.
...
Router: `/games` (hub) + `/games/rate-off` / `/games/higher-lower` / `/games/wordle` / `/games/quiz` / `/games/connections` / `/games/six-degrees`, all `requiresLogin`. Entry point: a controller-icon button in Home's results-actions row (`goToGames()`) — see the Grouped Result Hierarchy section above for why it moved into the slot the group-order button used to occupy.
...
Tests: `src/test/games/` — pure-module tests (`gameUtils`, `bracket`, `wordleClues`, `quizGenerator`, `connectionsGenerator`, `sixDegrees`, no mounting) plus one mount-based test per game component. Two recurring test-writing traps worth remembering if you add another game: (1) `new Date('YYYY-01-01')` parses as UTC midnight and shifts to the previous year/decade in this repo's negative-UTC-offset test environment — use a mid-month date in fixtures, not the 1st; (2) if a component sets its own initial state in `mounted()` rather than a computed or `created()`, a synchronous post-mount DOM assertion will see the pre-mount-hook render, one tick stale — prefer `created()` for pure data setup that doesn't touch the DOM.
