...
- **Reel Wordle** (`wordleClues.js` + `gameUtils.pickDailyEntry` + `ReelWordleGame.vue`) — one movie from the library, chosen **deterministically per calendar day** (`hashString(dateString)` seeds a small mulberry32 PRNG, indexes into the eligible-entries array — no server round trip, same puzzle for the same day on repeat visits). A type-ahead guess is scored against the target on year/decade/director/genre-overlap/runtime/your-rating, Wordle-style. **Overhauled (Jul 2026)** per feedback that the 6-guess Wordle cap didn't fit this format:
  ...
  - **"New Puzzle (practice)"** — `startNewPuzzle()` picks a fresh random (not date-seeded) target and resets progress in memory only, deliberately NOT persisted to `localStorage`, so testing/practice doesn't touch the daily puzzle's saved state. A reload always returns to the real daily puzzle; only `dailyTarget` (not the combined `target`) is watched for `loadPersistedGuesses`, and `persistGuesses`/`loadPersistedGuesses` are both skipped while `isManualPuzzle` is true.
