...
Router: `/games` (hub) + `/games/rate-off` / `/games/higher-lower` / `/games/wordle` / `/games/quiz`, all `requiresLogin`. Entry point: a controller-icon button next to Insights in Home's results-actions row (`goToGames()`).

Tests: `src/test/games/` — pure-module tests (`gameUtils`, `bracket`, `wordleClues`, `quizGenerator`, no mounting) plus one mount-based test per game component.

**Not built yet** (designed, deliberately deferred — see the ideas list from the Jul 2026 overnight pass): a Connections-style grouping puzzle (movies grouped by shared director/genre/decade/cast, NYT-Connections-style), a "six degrees" cast/crew pathfinder between two movies in the library, and per-game filters (e.g. restrict a Rate-Off bracket to one genre/decade/quick-link before starting).
