...
- **Reel Wordle** (`wordleClues.js` + `ReelWordleGame.vue`) — a random movie from the library each round; a type-ahead guess is scored against it on year/decade/director/genre-overlap/runtime/your-rating, Wordle-style. **Overhauled (Jul 2026)**, in two passes, per feedback that neither the 6-guess cap nor the once-a-day cadence fit this format ("it's not really Wordle"):
  ...
  - **Mobile horizontal-scroll fix (Jul 2026, bug report)**: the guess history used to be a 7-column CSS grid (`grid-template-columns: 2fr repeat(6, 1fr)`) with a hardcoded `min-width: 480px`, wider than a phone screen — every guess row scrolled sideways to be read. Replaced with a per-guess card (`.clue-row`) containing a `flex-wrap` row of labeled chips (`.clue-chip`, one per category), which reflows naturally to fit any width instead of forcing a fixed column layout. No `overflow-x` needed anymore.
