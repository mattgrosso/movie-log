...
**`BackLink.vue` hides the global Header (Jul 2026 fix)**: every game screen + the hub use `<BackLink>` as their sole top-of-page affordance (same "◀ Home"/"◀ Games" caret pattern as MovieDetail/RateMovie/Insights' `.home-link` — see "Unified Home/Back Affordance" above), but none of the 7 usages called `setShowHeader`, so the global "Cinema Roll" header rendered above the link instead of it standing alone at the top (reported as "the Home link is below the header"). Fixed centrally in `BackLink.vue` itself (`created()`/`beforeUnmount()` toggle `setShowHeader`) rather than repeating the pair in all 7 components — any future game automatically gets correct behavior just by using `<BackLink>`. Test: `src/test/games/BackLink.test.js`.
