...
- **Reel Wordle** (`wordleClues.js` + `ReelWordleGame.vue`) — a random movie from the library each round; a type-ahead guess is scored against it on year/decade/director/genre-overlap/runtime/your-rating, Wordle-style. **Overhauled (Jul 2026)**, in two passes, per feedback that neither the 6-guess cap nor the once-a-day cadence fit this format ("it's not really Wordle"):
  ...
  - **Input moved below the guess history (Jul 2026, bug report — "so I can see the most recent information as I'm typing")**: the text input + suggestions dropdown used to render at the TOP, above the (growing) guess-history grid, so the newest guess ended up furthest from the input as the list grew. Template order is now: target-clues → `.clue-grid` (oldest guess first, newest last, unchanged) → the input (`.guess-form`, when `status==='playing'`) / win banner (when won) → "New Puzzle" button. The newest guess row now sits immediately above wherever you're about to type next, chat-log style, instead of requiring a scroll back up past the whole history. (Not addressed: the `.suggestions` dropdown is still `position: absolute`, opening downward below the input — with the input now near the bottom of a long guess list, a tall suggestion list can extend past the viewport bottom; not reported as a problem yet, so left as-is rather than adding an upward-opening combobox pattern preemptively.)
...
- **New pure module: `src/assets/javascript/tieBreakTournament.js`** (store-free, same pattern the now-removed `games/bracket.js` used) — `findTiedGroup(sortedEntries, getScore)` expands outward from the first adjacent tie to the full **contiguous run** of equal-scored entries (ties are always contiguous in a sorted list); `createRoundRobinTournament(contestantIds)` builds every unique pair (N choose 2, no shuffling — deterministic); `currentMatch`/`recordMatchResult`/`isComplete` are an immutable state machine identical in spirit to a pick-a-winner bracket; `rankContestants` sorts by win count descending (falls back to original order for a tie-of-ties, e.g. a 3-cycle where everyone goes 1-1 — not worth resolving further); `tweakDeltaForRank(rank)` generalizes the old flat `-0.1` to `-0.1 * rank` (rank 0 = untouched, matching "winner never penalized").
- **Persisted across sessions, one match at a time, at the existing cadence.** The tournament state lives at `settings/tieBreakTournament` in Firebase — **deliberately not resolved in one sitting**. `Home.vue`'s `shouldShowTieBreakModal` only checks "does a tournament record exist" (regardless of complete/incomplete — see below) vs. the same daily quota (`settings.tieBreakTweak`/`lastTweak`) as before; it does NOT own creating or advancing the tournament. `TweakInline.vue` owns that via `ensureTournamentStarted` — each pick calls `recordMatchResult` + persists + re-stamps `lastTweak` exactly like the old single-pair flow did, so a big tie group surfaces one match per normal tiebreak appearance, indistinguishable in pacing from before, just spread across however many days it takes.
