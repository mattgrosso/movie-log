...
describe('MovieDetail — Custom List membership chips', () => {
  // Regression: the chips shipped referencing inList/toggleList before the
  // methods existed. Nothing caught it — lint and build both pass on a
  // template that calls a missing method; the page just renders blank.
  const LISTS = [
    { id: 'comfort', name: 'Comfort Watches', sortMode: 'manual', items: { 42: { at: 1, order: 0 } } },
    { id: 'kids', name: 'Show the Kids', sortMode: 'manual', items: {} }
  ]

  function mountWithLists () {
    ...
    const wrapper = shallowMount(MovieDetail, {
      global: {
        mocks: {
          $store: {
            state: { movieLog: {}, settings: { tags: { 'viewing-tags': {} } }, academyAwardWinners: {} },
            getters: { allMoviesAsArray: [], allMediaAsArray: [], customLists: LISTS },
            commit: vi.fn(),
            dispatch
          },
          $route: { params: { tmdbId: '42' }, query: {} },
  ...
  it('renders a chip per list and marks the ones holding this movie', async () => {
    const { wrapper } = mountWithLists()
    await wrapper.setData({ result: makeResult(), movie: makeResult().movie })

    const chips = wrapper.findAll('.movie-list-chip')
    expect(chips.length).toBe(2)
    expect(wrapper.vm.inList(LISTS[0])).toBe(true)   // movie 42 is a member
    expect(wrapper.vm.inList(LISTS[1])).toBe(false)
  })

  it('tapping a chip adds to a list it is not in, and removes from one it is', async () => {
    const { wrapper, dispatch } = mountWithLists()
    await wrapper.setData({ result: makeResult(), movie: makeResult().movie })

    wrapper.vm.toggleList(LISTS[1])
    expect(dispatch).toHaveBeenCalledWith('addToCustomList', { listId: 'kids', tmdbId: 42 })

    wrapper.vm.toggleList(LISTS[0])
    expect(dispatch).toHaveBeenCalledWith('removeFromCustomList', { listId: 'comfort', tmdbId: 42 })
  })
})
