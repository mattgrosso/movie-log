<template>
  <div class="favorite-actresses">
    ...
    <PersonModal
      v-if="showModal && selectedActress"
      :person="selectedActress"
      role-label="Actress"
      :rank="rankOfActress"
      :library-average="libraryAverageScore"
      @close="closeActressModal"
      @search="searchForActress"
...
export default {
  ...
  computed: {
    rankOfActress () {
      if (!this.selectedActress) return null;
      const index = this.topTenList.findIndex((p) => p.name === this.selectedActress.name);
