<template>
  <div class="favorite-producers">
    ...
    <PersonModal
      v-if="showModal && selectedProducer"
      :person="selectedProducer"
      role-label="Producer"
      :rank="rankOfProducer"
      :library-average="libraryAverageScore"
      @close="closeProducerModal"
      @search="searchForProducer"
...
export default {
  ...
  computed: {
    rankOfProducer () {
      if (!this.selectedProducer) return null;
      const index = this.topTenList.findIndex((p) => p.name === this.selectedProducer.name);
    ...
    async buildTopTwelveList () {
      ...
      this.peopleData = Object.entries(valueToMovies).map(([name, appearances]) => {
        ...
        return {
          ...
          weights: sortedAppearances.map(() => 1) // All weights 1 for producers
        };
      });
