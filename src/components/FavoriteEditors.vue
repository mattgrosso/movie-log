...
export default {
  ...
  computed: {
    rankOfEditor () {
      if (!this.selectedEditor) return null;
      const index = this.topTenList.findIndex((p) => p.name === this.selectedEditor.name);
