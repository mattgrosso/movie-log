<template>
  <div class="favorite-actors">
    <ul>
      <li v-for="entry in topTenList" :key="entry.name" class="favorite-list-item col-3" @click="openActorModal(entry)">
    ...
    <div v-if="showModal && selectedActor" class="actor-modal-overlay">
      <div class="actor-modal">
        <button class="close-btn" @click="closeActorModal">&times;</button>
        <h2>{{ selectedActor.name }}</h2>
        <div class="portrait-wrapper" v-if="selectedActor.details && selectedActor.details.profile_path">
          <img
            :src="`https://image.tmdb.org/t/p/w185${selectedActor.details.profile_path}`"
        ...
        <div class="modal-section">
          <h3>Rated Films:</h3>
          <ul class="films-list">
            <li v-for="film in selectedActor.entries" class="col-12" :key="film.movie.id">
              <strong>{{ film.movie.title }}</strong>
            </li>
          </ul>
        </div>
        <button class="search-btn col-12" @click="searchForActor">Search for this actor</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    bayesianAverage(list, weights = null) {
      // Weighted Bayesian average using blended performance/overall rating for actors
