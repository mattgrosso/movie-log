<template>
  <div class="favorite-writers">
    ...
    <PersonModal
      v-if="showModal && selectedWriter"
      :person="selectedWriter"
      role-label="Writer"
      :rank="rankOfWriter"
      :library-average="libraryAverageScore"
      @close="closeWriterModal"
      @search="searchForWriter"
...
export default {
  ...
  computed: {
    rankOfWriter () {
      if (!this.selectedWriter) return null;
      const index = this.topTenList.findIndex((p) => p.name === this.selectedWriter.name);
