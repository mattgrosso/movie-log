<template>
  <div class="favorite-writers">
    ...
    <div v-if="showModal && selectedWriter" class="writer-modal-overlay">
      <div class="writer-modal">
        <button class="close-btn" @click="closeWriterModal">&times;</button>
        <h2>{{ selectedWriter.name }}</h2>
        <div class="portrait-wrapper" v-if="selectedWriter.details && selectedWriter.details.profile_path">
          <img
            :src="`https://image.tmdb.org/t/p/w185${selectedWriter.details.profile_path}`"
        ...
        <div class="modal-section">
          <h3>Rated Films</h3>
          <ul class="films-list">
            <li v-for="film in selectedWriter.entries" :key="film.movie.id">
        ...
        <button class="search-btn" @click="searchForWriter">Search for this writer</button>
      </div>
    </div>
  </div>
</template>
  ...
  computed: {
    ...
    tunerLevers () {
      return [
        ...
        {
          key: 'storyWeight',
          label: 'Story vs. overall',
          value: this.storyWeight,
          min: 0,
          max: 1,
          step: 0.05,
