...
function tmdbGrabber(id, api_key) {
  ...
  for (var i = 0; i < pdata.length; i++) {
    if (companies.length < 2) {
      companies = pdata[i].name;
    } else {
      companies = companies + " | " + pdata[i].name;
    }
  }
...
function imdbGrabber(id) {
  ...
  do {
    var awards_data = UrlFetchApp.fetch(link).getContentText();
  } while (awards_data.length < 10);
  ...
  if (awards_data.length.toString() < 1) {
    awards_data = "nothing<tr>";
  } /*The "Uncle Buck" code for when a film has 0 awards.*/
  ...
  for (var a = 0; a < awards.length; a++) {
    ...
    if (awards[a][0] == "Win Oscar") {
      wins.push(awards[a][1]);
    }
    if (awards[a][0] == "Nomination Oscar") {
      noms.push(awards[a][1]);
    }
  }
  wins = wins.join(" | ");
  noms = noms.join(" | ");
...
function AwardsParse(awards) {
  ...
  if (awards[0][0].indexOf("Oscar") >= 0 || awards[1][0].indexOf("Oscar") >= 0 || awards[2][0].indexOf("Oscar") >= 0) {
    ...
    for (var i = 0; i < awards.length; i++) {
      if (awards[i][0].indexOf("Nominee") >= 0) {
        wins = i;
      }
    }
    /*Label Honorary Oscars*/
    for (var i = 0; i < awards.length; i++) {
      if ((awards[i][0].indexOf("Honor") >= 0) || (awards[i][0].indexOf("Special") >= 0) || (awards[i][0].indexOf("Technical Achievement") >= 0)) {
        awards[i][1] = "Honorary Oscar";
      }
    }
    /*Replace Columns 1 with Win and Nomination*/
    for (var i = 0; i < awards.length; i++) {
      if (i < wins) {
        awards[i][0] = "Win Oscar";
      } else {
        awards[i][0] = "Nomination Oscar"
      }
    }
    /*Replace Column 2 with catgeories*/
    for (var i = 0; i < awards.length; i++) {

      if ((awards[i][1].indexOf("Actor") >= 0) & (awards[i][1].indexOf("Lead") >= 0)) {
        awards[i][1] = "Best Actor - Lead";
      } else if ((awards[i][1].indexOf("Actress") >= 0) & (awards[i][1].indexOf("Lead") >= 0)) {
        awards[i][1] = "Best Actress - Lead";
      } else if ((awards[i][1].indexOf("Actor") >= 0) & (awards[i][1].indexOf("Supporting") >= 0)) {
        awards[i][1] = "Best Actor - Supporting";
      } else if ((awards[i][1].indexOf("Actress") >= 0) & (awards[i][1].indexOf("Supporting") >= 0)) {
        awards[i][1] = "Best Actress - Supporting";
      } else if ((awards[i][1].indexOf("Animat") >= 0) & (awards[i][1].indexOf("Feature") >= 0)) {
        awards[i][1] = "Best Animated Film";
      } else if ((awards[i][1].indexOf("Animat") >= 0) & (awards[i][1].indexOf("Short") >= 0)) {
        awards[i][1] = "Best Animated Short";
      } else if (awards[i][1].indexOf("Cinematography") >= 0) {
        awards[i][1] = "Best Cinematography";
      } else if (awards[i][1].indexOf("Costume") >= 0) {
        awards[i][1] = "Best Costumes";
      } else if ((awards[i][1].indexOf("Documentary") >= 0) & (awards[i][1].indexOf("Feature") >= 0)) {
        awards[i][1] = "Best Documentary";
      } else if ((awards[i][1].indexOf("Documentary") >= 0) & (awards[i][1].indexOf("Short") >= 0)) {
        awards[i][1] = "Best Documentary Short";
      } else if ((awards[i][1].indexOf("Sound") >= 0) & (awards[i][1].indexOf("Editing") >= 0)) {
        awards[i][1] = "Best Sound Editing";
      } else if (awards[i][1].indexOf("Editing") >= 0) {
        awards[i][1] = "Best Editing";
      } else if (awards[i][1].indexOf("Sound") >= 0) {
        awards[i][1] = "Best Sound Mixing";
      } else if (awards[i][1].indexOf("Foreign") >= 0) {
        awards[i][1] = "Best Foreign Film";
      } else if (awards[i][1].indexOf("International") >= 0) {
        awards[i][1] = "Best Foreign Film";
      } else if (awards[i][1].indexOf("Makeup") >= 0) {
        awards[i][1] = "Best Makeup";
      } else if ((awards[i][1].indexOf("Score") >= 0 | awards[i][1].indexOf("Scoring") >= 0) & !(awards[i][1].indexOf("Song") >= 0) & !(awards[i][1].indexOf("Adapted") >= 0)) {
        awards[i][1] = "Best Original Score";
      } else if ((awards[i][1].indexOf("Song") >= 0) & !(awards[i][1].indexOf("Scor") >= 0)) {
        awards[i][1] = "Best Original Song";
      } else if ((awards[i][1].indexOf("Production") >= 0) & (awards[i][1].indexOf("Design") >= 0)) {
        awards[i][1] = "Best Production Design";
      } else if ((awards[i][1].indexOf("Art") >= 0) & (awards[i][1].indexOf("Direction") >= 0)) {
        awards[i][1] = "Best Production Design";
      } else if ((awards[i][1].indexOf("Best") >= 0) & (awards[i][1].indexOf("Director") >= 0)) {
        awards[i][1] = "Best Director";
      } else if ((awards[i][1].indexOf("Best") >= 0) & (awards[i][1].indexOf("Direction") >= 0)) {
        awards[i][1] = "Best Director";
      } else if ((awards[i][1].indexOf("Best") >= 0) & (awards[i][1].indexOf("Directing") >= 0)) {
        awards[i][1] = "Best Director";
      } else if ((awards[i][1].indexOf("Visual") >= 0) & (awards[i][1].indexOf("Effects") >= 0)) {
        awards[i][1] = "Best Visual Effects";
      } else if ((awards[i][1].indexOf("Special") >= 0) & (awards[i][1].indexOf("Effects") >= 0)) {
        awards[i][1] = "Best Visual Effects";
      } else if ((awards[i][1].indexOf("Writing") >= 0) & ((awards[i][1].indexOf("Adapt") >= 0) || (awards[i][1].indexOf("Another") >= 0))) {
        awards[i][1] = "Best Adapted Screenplay";
      } else if ((awards[i][1].indexOf("Writing") >= 0) & ((awards[i][1].indexOf("Original") >= 0) || (awards[i][1].indexOf("Factual") >= 0) || (awards[i][1].indexOf("Directly") >= 0))) {
        awards[i][1] = "Best Original Screenplay";
      } else if ((awards[i][1].indexOf("Screenplay") >= 0) & (awards[i][1].indexOf("Adapt") >= 0)) {
        awards[i][1] = "Best Adapted Screenplay";
      } else if ((awards[i][1].indexOf("Screenplay") >= 0) & (awards[i][1].indexOf("Orig") >= 0)) {
        awards[i][1] = "Best Original Screenplay";
      } else if ((awards[i][1].indexOf("Writing") >= 0)) {
        awards[i][1] = "Best Screenplay";
      } else if ((awards[i][1].indexOf("Best") >= 0) & (awards[i][1].indexOf("Picture") >= 0)) {
        awards[i][1] = "Best Picture";
      } else if (awards[i][1].indexOf("Honor") >= 0) {
        awards[i][1] = "Honorary Oscar";
      } else {
        awards[i][1] = "Discontinued Oscar";
      }
    }
  }
